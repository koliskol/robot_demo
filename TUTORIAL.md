...
| Box/hand invisible once very close to the camera | Was Isaac Sim's default 1.0m near clipping plane - anything closer than 1m to the lens wasn't rendered at all. Fixed via `camera.set_clipping_range()` (`CAMERA_NEAR_CLIP_M = 0.02`) - confirmed live (an object ~0.4m away rendered as nothing before, visible after). If something still looks wrong up close after this, it's more likely the fixed camera angle just not pointing at it - see `CLAUDE.md`'s camera-mount note. |
