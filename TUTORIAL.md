...
| A dark shape fills most of the frame during a deep crouch + arm swing | That's the robot's own torso/shoulder self-occluding the head-mounted camera, not a bug - confirmed live. Tilting down further makes it worse, not better. Try less torso crouch (partial `I`/`K`), relying more on elbow lift (`J`/`L`) to keep the torso out of the camera's view. |
